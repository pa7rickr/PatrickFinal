...
const premium = JSON.parse(fs.readFileSync('./database/json/prem.json'))
...
if (mesejAnti.includes("://chat.whatsapp.com/")) {
	...
	setTimeout(() => {
		patrick.groupRemove(from, [kic]).catch((e) => {
			reply(`*PatrickBot Belum Menjadi Admin, Tidak Dapat Kick*`)
		})
	}, 3000)
	setTimeout(() => {
		patrick.updatePresence(from, Presence.composing)
...
if (bad.includes(messagesC)) {
	...
	setTimeout(() => {
		patrick.groupRemove(from, [Kick]).catch((e) => {
			reply(`ERROR: ${e}`)
		})
	}, 10)
	setTimeout(() => {
...
/******************* TebakGambar, TebakKata & Tebak Matematika PatrickBot  *******************/
        ...
        if (game.isMath(from, tebakgambar) && isRegistered){
...
if (!isGroup && isRegistered && isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', [time], '[\x1b[1;32mInPrivate\x1b[1;37m]=', color([command]), 'from', color(sender.split('@')[0]), 'args :', color(args.length))
if (isCmd && isRegistered && isGroup) console.log('\x1b[1;31m~\x1b[1;37m>', [time], '[\x1b[1;32mInGroup\x1b[1;37m]=', color([command]), 'from', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
if (isCmd) cmdadd()
switch (command) {
	//==========================================BATES NGAB==========================================\\
	    case 'help':
...
╰───────────────${a}
       ...
       ~ ${botName} 2021`
		...
		teksnya =
			`${a}╭────${a}「 *Feature Group* 」
${a}│ ❏ Antilink: ${AntilinkMode}
│ ❏ Antivirtex: ${AntivirtexMode}
│ ❏ Antibadword: ${BadwordMode}
│ ❏ Welcome: ${WelcomeMode}
│ ❏ SimiSimi: ${SimiMode}
...
*Example: ${prefix}enable antilink*`
		patrick.sendMessage(from, teksnya, text, {
			quoted: ftoko
		})
		break
		//==========================================BATES NGAB==========================================\\
...
${a}│ ❏  Total Premium User : ${prem.length}${a}
		...
		for (let _ of patrick.chats.all()) {
            ...
            case 'delprem':
                ...
                if (mentioned.length !== 0){
                    for (let i = 0; i < mentioned.length; i++){
                        ...
                        fs.writeFileSync('./database/json/prem.json', JSON.stringify(premium))
                    }
                    reply('Sukses')
                } else {
		...
		try {
			...
			teksnya = body.slice(1)
			reply(ind.wait())
			patrickganss = await getBuffer(`https://dapuhy-api.herokuapp.com/api/others/styletext?text=${q}&apikey=LOd5PF5QZMs7XE2`)
        ...
        anu = await fetchJson(`https://dapuhy-api.herokuapp.com/api/fun/tebakgambar?apikey=${dappaApi}`)
        ...
        game.addSPK(from, anihs, gamewaktus, math)
        limitAdd(sender)
        break
		case 'tebaksiapa':
		case 'tebaksiapakah':
        ...
        anuh = await fetchJson(`https://api.zeks.xyz/api/siapa-aku?apikey=${zeksApi}`)
        reply(`*Jawablah Soal Berikut Ini*\n\n*Soal :* ${anuh.data.soal}\n\nWaktu : ${gamewaktu}s`)
		...
		let femm = await fetchJson(`https://api.zeks.xyz/api/family100?apikey=${zeksApi}`)
        reply(`*Jawablah Soal Berikut Ini*\n\n*Soal :* ${femm.data.pertanyaan}\n*Total Jawaban :* ${femm.data.jawaban.array.length}\n\nWaktu : ${gamewaktu}s`)
        let anoh = femm.data.jawaban.array
        let rgfds = []
        for (let i of anoh){
		...
		if (isBanned) return reply(ind.baned())
		if (isLimit(sender)) return reply(limitend(pushname))
		getanu = await fetchJson(`https://api.zeks.xyz/api/memeindo?apikey=${zeksApi}`)
		patrick.sendMessage(from, getanu.result, text, {
			quoted: fgclink
		})
		await limitAdd(sender)
		break
...
╰─────「 *${botName}* 」`
		...
		if (args[0] === 'welcome') {
			if (isWelcome) return reply(`*Fitur ${command} sudah aktif*`)
			antilink.push(from)
		...
		if (args[0] === 'nsfw') {
			if (!isNsfw) return reply(`*Fitur ${command} belum aktif*`)
			nsfw.splice(from, 1)
			fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
			reply('❬ *SUKSES* ❭ *MENONAKTIFKAN FITUR NSFW DI GROUP INI*')
		} else
		if (args[0] === 'simi') {
			if (!isSimi) return reply(`Fitur ${command} belum aktif`)
			if (!isOwner) return reply(ind.ownerb())
			samih.splice(from, 1)
			fs.writeFileSync('./database/simi.json', JSON.stringify(antivirtex))
		...
		if (args[0] === 'welcome') {
			if (!isWelcome) return reply(`*Fitur ${command} belum aktif*`)
			antilink.splice(from, 1)
			fs.writeFileSync('./database/json/welcome.json', JSON.stringify(antilink))
			reply('❬ *SUKSES* ❭ *MENONAKTIFKAN FITUR WELCOME|LEFT DI GROUP INI*')
		} else
		if (args[0] === 'leveling') {
			if (!isLevelingOn) return reply(`*Fitur ${command} belum aktif*`)
		...
		try {
			...
			if (isBanned) return reply(ind.baned())
			if (isLimit(sender)) return reply(ind.limitend(pushname))
			if (args.length < 1) return reply(`Example: ${prefix + command} https://vm.tiktok.com/ZSJkHUCwK/`)
			reply(ind.wait())
			link = args.join(' ')
		...
		//==========================================BATES NGAB==========================================\\
	default:
	    if (isRegistered && body.startsWith(`${prefix + command}`)) {
