...
const sewa = JSON.parse(fs.readFileSync('./database/json/sewa.json'))
...
const sendStickerFromUrl = async (to, url) => {
	...
	download(url, namea, async function() {
		...
		exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
			...
			exec(`webpmux -set exif ./sticker/take${lehh}.exif ${asw} -o ${asw}`, async (error) => {
...
const sendStickerUrl = async (to, url) => {
	...
	download(url, namea, async function() {
		...
		exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
			...
			exec(`webpmux -set exif ./sticker/take${lohh}.exif ${asw} -o ${asw}`, async (error) => {
				let media = fs.readFileSync(asw)
...
*Untuk menonaktifkannya, Ketik ${prefix}disable*`
		...
		if (checkATMuser(sender) <= uang4) return replyWithFakeLink(`Pastikan Ub Kamu Tercukupi\nHarga: $${uang4}`)
...
${a}>  Total Premium User : ${prem.length}${a}
		...
		case 'addsewa':
         ...
         linked = `${args[1].replace('https://chat.whatsapp.com/','')}`
